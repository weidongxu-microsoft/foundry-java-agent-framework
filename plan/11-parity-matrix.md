...
| Multimodal content (image/data in + out) | ✅ | ✅ | ⚠️ | `core/chat/DataContent.java`, `AgentResponseHandler`, `RawDevelopMiddleware`, `OpenAIResponsesChatClient` | **In:** `input_file`/`input_image` → `DataContent` + text breadcrumb; text model skips binary (never sent upstream). Inline base64 **or** a server-side-fetched `http(s)` `file_url` (blob SAS) — the latter sidesteps the ~18 MB gateway request-size limit. **Vision-in:** renderable images (`image/jpeg,png,gif,webp`) map → `input_image` in `OpenAIResponsesChatClient` (RAW excluded). **Out (items #1–#4):** host pipeline returns a developed JPEG as a `data:` URL in `output_text`; item #2/#3 adds a vision-advice sub-call (adjustments), item #4 auto lens correction (Lensfun). **All verified on the hosted agent** via blob SAS (`plan/19`). **Remaining:** native `output_image` content part. |
