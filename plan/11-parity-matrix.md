...
| Multimodal content (image/data in + out) | ✅ | ✅ | ⚠️ | `core/chat/DataContent.java`, `AgentResponseHandler`, `RawDevelopMiddleware` | **In:** `input_file`/`input_image` → `DataContent` + text breadcrumb; text model skips binary (never sent upstream). Inline base64 **or** a server-side-fetched `http(s)` `file_url` (blob SAS) — the latter sidesteps the ~18 MB gateway request-size limit. **Out (item #1):** host pipeline returns a developed JPEG as a `data:` URL in `output_text`. **Verified end-to-end on the hosted agent** via blob SAS (`plan/19`). **Remaining:** native `output_image` content part + model vision-in (`ResponseInputImage`) for the advice sub-call (item #2/#3). |
...
7. **Multimodal content (image/data in + out)** ⚠️ — `DataContent` in core ✅; `input_image`/`input_file` accepted in `AgentResponseHandler` ✅ (inline base64 **or** server-side-fetched `http(s)` `file_url`/blob SAS, which sidesteps the ~18 MB gateway request-size limit); host-pipeline image-out as `data:` URL ✅ (item #1, **verified on the hosted agent** via blob SAS). **Remaining:** native `output_image` content part + `input_image` mapping in `OpenAIResponsesChatClient` for the vision advice sub-call (item #2/#3). Motivating workload: RAW-photo tool (`plan/19`).
