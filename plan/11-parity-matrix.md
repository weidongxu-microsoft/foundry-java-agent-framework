...
| Content types (text / func call / result / approvals / data) | ✅ | ✅ | ✅ | `core/chat/*Content.java` | Text/FunctionCall/FunctionResult/ToolApproval + **`DataContent`** (bytes + media type + name; MAF parity). |
| Multimodal content (image/data in + out) | ✅ | ✅ | ⚠️ | `core/chat/DataContent.java`, `AgentResponseHandler`, `RawDevelopMiddleware`, `OpenAIResponsesChatClient` | **In:** `input_file`/`input_image` → `DataContent` + text breadcrumb; text model skips binary (never sent upstream). Inline base64 **or** a server-side-fetched `http(s)` `file_url` (blob SAS) — the latter sidesteps the ~18 MB gateway request-size limit. **Vision-in:** renderable images (`image/jpeg,png,gif,webp`) map → `input_image` in `OpenAIResponsesChatClient` (RAW excluded). **Out (items #1–#5):** host pipeline returns a developed JPEG as a `data:` URL in `output_text`; item #2/#3 adds a vision-advice sub-call (adjustments), item #4 auto lens correction (Lensfun), item #5 AI crop advice (app-side `ImageCrop`). **All verified on the hosted agent** via blob SAS (`plan/19`). **Remaining:** native `output_image` content part. |
...
7. **Multimodal content (image/data in + out)** ⚠️ — `DataContent` in core ✅; `input_image`/`input_file` accepted in `AgentResponseHandler` ✅ (inline base64 **or** server-side-fetched `http(s)` `file_url`/blob SAS, which sidesteps the ~18 MB gateway request-size limit); vision-in mapping (`input_image`) in `OpenAIResponsesChatClient` ✅ (renderable images only; RAW excluded; the multi-part message sets `type: "message"` explicitly or the Responses API returns `400 Invalid value: ''`); host-pipeline image-out as `data:` URL ✅ (items #1–#5: neutral develop, vision-advice loop, auto lens correction via Lensfun, and app-side AI crop advice — **verified on the hosted agent** via blob SAS). **Remaining:** native `output_image` content part. Motivating workload: RAW-photo tool (`plan/19`).
