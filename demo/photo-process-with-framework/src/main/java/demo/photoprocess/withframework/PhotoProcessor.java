...
/**
 * Pure-JDK JPEG crop — no third-party image library. Identical in both the with-framework and
 * without-framework projects: it is the shared "real work", not the thing being compared.
 *
 * <p>Reads a JPEG, clamps the requested rectangle to the image bounds, copies the sub-image into a
 * standalone {@code TYPE_INT_RGB} buffer (so the result no longer shares the source raster and
 * encodes cleanly as JPEG), and re-encodes it.
 */
